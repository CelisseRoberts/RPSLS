import random

user_wins = 0
computer_wins = 0 

options = ["rock", "paper", "scissors", "lizard", "spock"]

while True:
    user_input = input('Type rock, paper, scissors, lizard, spock or Q to quit: ').lower()
    if user_input == "q": 
        break

    if user_input not in options:
        continue  


    random_number = random.randint(0,5)
    # rock: 0, paper: 1, scissors: 2, lizard: 3, spock: 4
    computer_selection = options[random_number]
    print("Computer selected", computer_selection)

    if user_input == "rock" and computer_selection == "scissors":
        print("You won!")
        user_wins += 1 
    
    elif user_input == "scissors" and computer_selection == "paper":
        print("You won!")
        user_wins += 1

    elif user_input == "paper" and computer_selection == "rock":
        print("You won!")
        user_wins += 1

    elif user_input == "rock" and computer_selection == "lizard":
        print("You won!")
        user_wins += 1

    elif user_input == "lizard" and computer_selection == "spock":
        print("You won!")
        user_wins += 1

    elif user_input == "spock" and computer_selection == "scissors":
        print("You won!")
        user_wins += 1

    elif user_input == "scissors" and computer_selection == "lizard":
        print("You won!")
        user_wins += 1 

    elif user_input == "lizard" and computer_selection == "paper":
        print("You won!")
        user_wins += 1 

    elif user_input == "paper" and computer_selection == "spock":
        print("You won!")
        user_wins += 1 

    elif user_input == "spock" and computer_selection == "rock":
        print("You won!")
        user_wins += 1

    #elif user_input == computer_selection: 
        #print("You tied!")
        #user_wins += 0
        #computer_wins += 0

    else: 
        print("You lost!")
        computer_wins += 1 




print("Thanks for playing! See you next time.")
